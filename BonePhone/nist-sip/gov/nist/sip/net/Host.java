...
public class Host extends NetObject  implements HostAddrTypes {
	...
	// (sfo) recognize IPv6 reference
	if ( hostName.startsWith("[") && hostName.endsWith("]") )
	{
	   String s = hostName.substring(1,hostName.length() - 1);
	   try
	   {
		inetAddress = Inet6Address.getByName(s);
		this.addressType = IPV6REFERENCE;
		this.address = s;
		return;
	   }
	   catch (UnknownHostException e)
	   {
		throw new IllegalArgumentException("Bad host name or address");
	   }
	}
	...
	// (sfo) trim IPv6 reference
	if ( addrType == IPV6REFERENCE )
	{
	...
	// (sfo) encode IPv6 reference too
        if (addressType == HOSTNAME) return hostname.trim();
	else if ( addressType == IPV6REFERENCE) return "["+address.trim()+"]";
