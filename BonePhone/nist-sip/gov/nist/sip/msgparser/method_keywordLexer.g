...
class method_keywordLexer extends Lexer;
 ...
 EXCLAMATION | PERCENT |  BACK_QUOTE | DOT | TILDE  | STAR  )+
{
	if (! keywordsInitialized) {
		...
		addKeyword(REGISTER,SIPRequestTypes.REGISTER);
		addKeyword(ACK, SIPRequestTypes.ACK )	 ; 
		addKeyword(OPTIONS,SIPRequestTypes.OPTIONS)  ;  	
		addKeyword(BYE, SIPRequestTypes.BYE)      	 ;
		addKeyword(INVITE,SIPRequestTypes.INVITE)    ;
		addKeyword(CANCEL,SIPRequestTypes.CANCEL)  	 ;
		addKeyword(SIP,SIPKeywords.SIP) 		 ;
	}
	...
	if (sipKeyword != null) {
		...
		$setType(tokenval);
		if ( tokenval == SIP) selectLexer("charLexer");
	}  
}
;
