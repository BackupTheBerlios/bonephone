...
class via_parmsLexer extends Lexer;
...
ID :(  ALPHA | DIGIT | UNDERSCORE  | PLUS  | MINUS  | QUOTE  | EXCLAMATION  | 
	PERCENT  |  BACK_QUOTE  | DOT  | 	TILDE   | STAR  )+ 
{            
	 /**
        * Initialize the priority Lexer keywords.
        */
	if (lexerState == 1) {
		// The equals has been seen -- dont match keywords.
		$setType(ID);
	} else {
          if (! keywordsInitialized)  {
                ...
                addKeyword(HIDDEN, SIPKeywords.HIDDEN);
                addKeyword(RECEIVED, SIPKeywords.RECEIVED);
