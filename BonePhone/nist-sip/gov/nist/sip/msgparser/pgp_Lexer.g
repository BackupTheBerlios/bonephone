 ...
 EXCLAMATION | PERCENT |  BACK_QUOTE | DOT | TILDE  | STAR  )+
{
	if (! keywordsInitialized) {
		...
		addKeyword(VERSION,SIPKeywords.VERSION)		; 
		addKeyword(ALGORITHM, SIPKeywords.ALGORITHM)      	;
		addKeyword(PUBKEY,SIPKeywords.PUBKEY)   		;
		addKeyword(NONCE,SIPKeywords.NONCE)  		;
		addKeyword(SIGNED_BY,SIPKeywords.SIGNED_BY) 	;
		addKeyword(SIGNATURE,SIPKeywords.SIGNATURE) 	;
	}
