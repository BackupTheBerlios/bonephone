...
class priority_Lexer extends Lexer;
 ...
 EXCLAMATION | PERCENT |  BACK_QUOTE | DOT | TILDE  | STAR  )+
{
	...
	*/
	...
	if (! keywordsInitialized)  {
		...
		addKeyword(EMERGENCY, PriorityKeywords.EMERGENCY   );
		addKeyword(URGENT, PriorityKeywords.URGENT 	  );
		addKeyword(NORMAL, PriorityKeywords.NORMAL  	  );
		addKeyword(NON_URGENT, PriorityKeywords.NON_URGENT );
	}
